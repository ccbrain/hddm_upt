...
class HDDM(HDDMBase):
    ...
    def _create_stochastic_knodes_info(self, include):
        ...
        if 'z' in include:
            ...
            #knodes.update(self._create_family_trunc_normal_NEW('z', lower=-2, upper=2, value=0,std_lower=1e-4,std_upper=1,std_value=.1))
            #knodes.update(self._create_family_trunc_normal('z', lower=0, upper=1, value=0.001,std_lower=1e-4,std_upper=1,std_value=.1))
            #knodes.update(self._create_family_invlogit('z', value=.5, g_tau=0.5**-2, std_std=0.05))
    ...
    def _create_stochastic_knodes_noninfo(self, include):
        ...
        if 'z' in include:
            ...
            #knodes.update(self._create_family_invlogit('z', value=.6, g_tau=2**-2, std_std=0.5))
            #knodes.update(self._create_family_gamma_gamma_hnormal('z', g_mean=0.1, g_std=0.1, std_std=0.1, std_value=0.1, value=0.1))
            knodes.update(self._create_family_trunc_normal_NEW('z', lower=-1, upper=1, value=0,std_lower=1e-4,std_upper=1,std_value=.1))
        if 'p_outlier' in include:
