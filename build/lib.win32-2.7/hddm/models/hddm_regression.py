        ...
        for reg in self.model_descrs:
            ...
            for inter, param in zip(intercept, reg['params']):
                if inter:
                    ...
                    if param == 'z_C(session)[T.2]':
                    #param_lookup = param[:param.find('_')]
                        #reg_family = self.(param,lower=-.4,upper=.4,std_lower=1e-4,std_upper=1,std_value=.1,value=0)
                        reg_family = self._create_family_trunc_normal_NEW(param,lower=-.4,upper=.4,std_lower=1e-4,std_upper=1,std_value=.1,value=0)
#                        reg_family = Knode(pm.Uniform, 'z', lower=-0.5, upper=0.5, value=0)
